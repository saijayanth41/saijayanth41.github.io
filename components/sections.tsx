...
export function AboutSection() {
  return (
    <Section id="about" eyebrow="About" title="Backend, cloud, and DevOps work for systems that need to stay correct.">
      <Card className="max-w-4xl">
        <p className="text-lg leading-8 text-slate-300">
          I am a Software Engineer with 2.5+ years of experience developing and supporting enterprise healthcare
          applications, with hands-on experience in .NET Core, REST APIs, SQL Server, Azure DevOps, AWS, Azure, Docker,
          Kubernetes, Terraform, CI/CD, and production support.
        </p>
        <p className="mt-5 text-lg leading-8 text-slate-300">
          I have worked on Medicare and Medicaid systems where reliability, data correctness, and secure engineering
          matter. My work spans backend APIs, cloud infrastructure, automation, deployment pipelines, defect resolution,
          ...
          <Card key={item.company}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-base font-semibold text-cyan-200">{item.company}</p>
    ...
    >
      <div className="grid gap-5 lg:grid-cols-2">
        {projects.map((project, index) => (
          <Card key={project.name} className={index === 0 ? "lg:col-span-2" : ""}>
            ...
            <p className="mt-5 font-mono text-xs uppercase tracking-[0.18em] text-slate-500">Engineering signal</p>
...
export function SkillsSection() {
  return (
    <Section id="skills" eyebrow="Skills" title="Backend-first skills with deployable cloud and DevOps coverage.">
...
export function CertificationsSection() {
  return (
    <Section id="certifications" eyebrow="Certifications" title="Credentials that support the cloud and platform story.">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {certifications.map((certification) => (
          <div key={certification} className="rounded-2xl border border-line bg-white/[0.035] p-5">
            <p className="font-semibold leading-7 text-slate-200">{certification}</p>
          </div>
        ))}
      </div>
    </Section>
...
export function OpenSourceSection() {
  return (
    <Section id="open-source" eyebrow="Open Source" title="Open-source contribution with practical framework documentation value.">
      <Card className="max-w-4xl">
        ...
        <p className="mt-4 leading-8 text-slate-300">
          Improved troubleshooting documentation for OpenAPI/GetDocument behavior in ASP.NET Core, following the GitHub
          contribution workflow for a major Microsoft-maintained framework repository.
        </p>
        <a
...
export function ContactSection() {
  return (
    <Section id="contact" eyebrow="Contact" title="Open to software engineering, cloud, and DevOps roles.">
      <Card className="grid gap-8 lg:grid-cols-[1fr_1.2fr]">
        <div>
          <p className="text-lg leading-8 text-slate-300">
            Based in {profile.location}. Strong fit for backend engineering, cloud infrastructure, DevOps automation,
            production support, and healthcare technology teams that value reliable delivery.
          </p>
        </div>
